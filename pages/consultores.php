...
(function () {
    ...
    // ─── flag: si hay un modal abierto, no recargar la lista (evita cerrar el modal) ───
    ...
    function mostrarErr(inp, el, msg) {
        if (inp) { inp.classList.add('is-error'); inp.classList.remove('is-valid'); }
        if (el)  { el.innerHTML = '<i class="fas fa-exclamation-circle"></i> ' + msg; el.classList.add('show'); }
    }
    function ocultarErr(inp, el) {
        if (inp) { inp.classList.remove('is-error'); inp.classList.add('is-valid'); }
    ...
    function bindCapitalizar(id) {
        ...
        el.addEventListener('blur', function () {
            ...
            if (this.value !== '') this.value = capitalizar(this.value, false);
        });
    }
    ...
    function cargarConsultores() {
        // No recargar si hay un modal de edición/creación abierto
        if (modalAbierto) return;

        fetch('includes/ajax/consultores.php?action=listar', { credentials: 'same-origin' })
        .then(function (r) { return r.json(); })
        .then(function (data) {
            if (data.success) renderLista(data.consultores);
        })
        .catch(function () {
            if (typeof mostrarToast === 'function') mostrarToast('Error cargando consultores', 'error');
        });
    }
    ...
    function renderLista(consultores) {
        ...
        var html = '<table class="consultores-table"><thead><tr>' +
            '<th>Nombre</th><th>Apellidos</th><th>País</th><th>Teléfono</th>' +
            '<th>Usuario</th><th>Estado</th><th>Último Acceso</th><th>Acciones</th>' +
            '</tr></thead><tbody>';
        ...
        container.querySelectorAll('.btn-action-toggle').forEach(function (btn) {
            btn.addEventListener('click', function () {
                ...
                mostrarConfirm(
                    accion === 'suspender' ? 'Suspender Consultor' : 'Activar Consultor',
                    '¿Estás seguro de <strong>' + accion + '</strong> al consultor <strong>' + nombre + '</strong>?',
                    accion === 'suspender' ? 'warning' : 'success',
        ...
        container.querySelectorAll('.btn-action-delete').forEach(function (btn) {
            btn.addEventListener('click', function () {
                ...
                mostrarConfirm(
                    'Eliminar Consultor',
                    '¿Estás seguro de <strong>eliminar</strong> al consultor <strong>' + nombre + '</strong>?<br><small style="color:#6b7280;">Esta acción no se puede deshacer.</small>',
                    'danger',
    ...
    // =====================================================
    // ACCIONES: usan el mismo endpoint consultores.php
    // =====================================================
    function toggleEstado(id) {
        var fd = new FormData();
    ...
    // Refresca la lista automáticamente para ver cambios
    // del consultor (edita su propio perfil) y del admin
    // (desde opciones-sistema).
    // Si hay un modal abierto, el poll se salta para no
    // interrumpir al admin mientras edita.
    // =====================================================
    cargarConsultores();
    setInterval(cargarConsultores, 10000);
