 ...
 *
 * Tabs dinámicos: GENERAL / ASESOR / DELEGADO
 * Cada tab carga sus propios filtros
 * Filtros combinados tipo LISTA (como en Dashboard)
 */
    ...
    <div class="filters-bar" id="filtersBarEstadisticas" style="flex-shrink:0;">
        ...
        <div class="est-tab-content active" id="tab-general">
            <div class="filters-row" style="flex-wrap:wrap;">
                <span class="filters-row-label" style="width: 86px;"><i class="fas fa-file-alt"></i> Formulario:</span>
                <select class="filter-select" id="estFormularioGeneral" style="min-width:220px;">
                    <option value="">Todos los Formularios</option>
                </select>

                <span class="filters-row-label" style="margin-left:12px;"><i class="fas fa-calendar"></i> Fecha:</span>
        ...
        <div class="est-tab-content" id="tab-asesor" style="display:none;">
            <div class="filters-row" style="flex-wrap:wrap;">
                <span class="filters-row-label" style="width: 86px;"><i class="fas fa-headset"></i> Asesor:</span>
        ...
        <div class="est-tab-content" id="tab-delegado" style="display:none;">
            <div class="filters-row" style="flex-wrap:wrap;">
                <span class="filters-row-label" style="width: 86px;"><i class="fas fa-user-shield"></i> Delegado:</span>
...
<style>
/* =====================================================
   TABS PROFESIONALES (Estilo pestañas)
   ===================================================== */
.est-tabs-container {
...
.est-tab-item:hover {
    color: #07325A;
    background: rgba(7, 50, 90, 0.02);
}

.est-tab-item.active {
    color: #07325A;
    border-bottom-color: #07325A;
    background: rgba(7, 50, 90, 0.03);
}
...
(function () {
    ...
    var STATE = {
        ...
        subFilters: {
            curso: '', pais: '', ciudad: '', metodo_pago: '', web: ''
        }
    };
    ...
    function cambiarTab(tabName) {
        ...
        Array.prototype.forEach.call(DOM.tabContents, function (content) {
        ...
        // Limpiar filtros (incluyendo visual)
    ...
    // =====================================================
    // LIMPIAR FILTROS (completo - data + visual)
    // =====================================================
    function limpiarFiltrosCompleto() {
        ...
        if (STATE.currentTab === 'general') {
            ...
            DOM.estFechaDesdeGeneral.value = '';
            DOM.estFechaHastaGeneral.value = '';
            DOM.estTendenciaGeneral.value = 'dia';
            DOM.estFormularioGeneral.value = '';
            DOM.estFormularioGeneral.classList.remove('active-filter');
        } else if (STATE.currentTab === 'asesor') {
            STATE.filters.asesor = { asesor: '', fecha_desde: '', fecha_hasta: '', tendencia: 'dia' };
            DOM.estAsesor.value = '';
            DOM.estFechaDesdeAsesor.value = '';
            DOM.estFechaHastaAsesor.value = '';
            DOM.estTendenciaAsesor.value = 'dia';
            DOM.estAsesor.classList.remove('active-filter');
        } else if (STATE.currentTab === 'delegado') {
            STATE.filters.delegado = { delegado: '', fecha_desde: '', fecha_hasta: '', tendencia: 'dia' };
            DOM.estDelegado.value = '';
            DOM.estFechaDescuDelegado.value = '';
            DOM.estFechaHastaDelegado.value = '';
            DOM.estTendenciaDelegado.value = 'dia';
            DOM.estDelegado.classList.remove('active-filter');
        }

        // Limpiar sub-filtros (data + visual)
        STATE.subFilters = { curso: '', pais: '', ciudad: '', metodo_pago: '', web: '' };
        DOM.estFilterCurso.value = '';
        DOM.estFilterPais.value = '';
        DOM.estFilterCiudad.value = '';
        DOM.estFilterMetodoPago.value = '';
        DOM.estFilterWeb.value = '';
        
        // Remover clase active-filter de todos los sub-filtros
        ...
        fetch('includes/ajax/get_estadisticas.php?type=filtros&' + qs, { credentials: 'same-origin' })
            ...
            .then(function (data) {
                if (data && data.success && data.filtros) {
                    // Rellenar selects con los valores dinámicos
    ...
    function llenarSelect(el, valores, placeholder) {
        ...
        // Si la opción seleccionada no existe en los nuevos valores, limpiar
        if (cv && !valores.includes(cv)) {
            el.value = '';
    ...
    function actualizarResumen(r) {
        ...
        document.getElementById('estDelegados').textContent = (r.asesores || 0).toLocaleString();
    ...
    function renderTendencia(data) {
        ...
        if (modo === 'dia') {
            serie.forEach(function (d) { labels.push(formatearFecha(d.dia)); valores.push(d.total); });
        } else if (modo === 'semana') {
            serie.forEach(function (d) { labels.push('Sem ' + formatearFecha(d.inicio_semana)); valores.push(d.total); });
        } else if (modo === 'mes') {
            serie.forEach(function (d) { labels.push(d.mes_nombre || d.periodo); valores.push(d.total); });
        } else {
            serie.forEach(function (d) { labels.push(d.periodo); valores.push(d.total); });
        }
